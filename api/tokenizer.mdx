...
Calculates the average number of bytes per token for a dataset.

```python
from nanochat.tokenizer import get_token_bytes

avg_bytes = get_token_bytes(tokenizer)
...
<ResponseField name="avg_bytes" type="float">
  Average number of bytes encoded per token
</ResponseField>
